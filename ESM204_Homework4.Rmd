...
```{r}
model<- lm(damages ~ warming + warming^2 -1, data = damage)
...
ggplot() +
  geom_point(data = damage_df, aes(x = warming, y = damages)) +
  #geom_abline(data = pred_damages) +
  geom_abline(mapping = aes(x = warming_df, y = pred_damages), intercept = 0, slope = model$coefficients) #this is wrong but good enough for now 
  theme_classic()
